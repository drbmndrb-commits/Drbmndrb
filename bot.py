...
def get_check_menu():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("💳 Stripe 2", callback_data="check_str2"),
        InlineKeyboardButton("💳 Stripe 3", callback_data="check_str3"),
        InlineKeyboardButton("💳 Stripe 4", callback_data="check_str4"),
...
def get_admin_panel():
    ...
    keyboard.add(
        ...
        InlineKeyboardButton("📁 Upload Gateway", callback_data="admin_upload_gateway"),
        InlineKeyboardButton("📂 Upload File Check", callback_data="admin_upload_check"),
...
def format_result(gateway_name, card, status, response, exec_time):
    ...
    # تنسيق البطاقة للنسخ
    copy_card = f"{card_number}|{month}|{year}|{cvv}"
    copy_bin = card_number[:6]
    
    # استخراج BIN info بشكل منظم
    bin_lines = bin_info.split('\n') if bin_info and '❌' not in bin_info else []
    bin_type = "Unknown"
    bin_bank = "Unknown"
    bin_country = "Unknown"
    ...
    for line in bin_lines:
        if 'Type:' in line:
            bin_type = line.replace('Type:', '').strip()
        elif 'Bank:' in line:
            bin_bank = line.replace('Bank:', '').strip()
        elif 'Country:' in line:
            bin_country = line.replace('Country:', '').strip()
    
    # تنسيق BIN info
    if '❌' not in bin_info:
        bin_display = bin_info.replace('━━━━━━━━━━━━━━━\n🔢 BIN:', '').strip()
    else:
        bin_display = "❌ Cannot get BIN info"
...
[ϟ] BIN: {copy_bin}
[ϟ] 𝐁𝐚𝐧𝐤: {bin_bank}
[ϟ] 𝐂𝐨𝐮𝐧𝐭𝐫𝐲: {bin_country}
━━━━━━━━━━━━━━━
Taken ➜ {exec_time:.2f}s
━━━━━━━━━━━━━━━
Dev ➜ @S3s_A
"""
...
def check_mass_cards(user_id, cards, gateway_func, gateway_name, message_id, stop_key):
    ...
    def update_display():
        ...
        kb.add(
            ...
            InlineKeyboardButton(f"⚠️ Tech Errors: {consecutive_errors}/{MAX_CONSECUTIVE_ERRORS}", callback_data="none"),
    ...
    for i, card in enumerate(cards, 1):
        ...
        try:
            ...
            is_approved = "Charged" in result or "✅" in result or "Approved" in result or "Insufficient" in result
            is_declined = "Declined" in result or "❌" in result or "card was declined" in result.lower()
            is_error = not is_approved and not is_declined
            ...
            if is_approved:
                ...
                error_msg = f"⚠️ **Technical Error**\n\n"
                error_msg += f"💳 Card: {card[:10]}...\n"
                error_msg += f"📡 Response: {result[:50]}\n"
                error_msg += f"⚠️ Consecutive Errors: {consecutive_errors}/{MAX_CONSECUTIVE_ERRORS}"
                bot.send_message(user_id, error_msg)
                update_stats(user_id, False)

                if consecutive_errors >= MAX_CONSECUTIVE_ERRORS:
                    error_msg = f"⚠️ **GATEWAY ERROR DETECTED!**\n\n"
                    error_msg += f"❌ {consecutive_errors} consecutive technical errors\n"
                    error_msg += f"💳 Last card: {card[:10]}...\n"
                    error_msg += f"📡 Response: {result[:50]}\n\n"
                    error_msg += f"🛑 Check stopped to avoid wasting points"
                    bot.send_message(user_id, error_msg)
                    break
...
❌ Declined: {declined}
📊 Total: {total}
⚠️ Technical Errors: {consecutive_errors}"""
...
def cmd_str2(m):
    user_id = m.from_user.id
    card = m.text.replace('/str2', '').strip()
    if not card:
        bot.reply_to(m, "⚠️ /str2 number|month|year|cvv")
        return
    check_single_card(user_id, card, Stripe2, "Stripe 2")
...
def cmd_str4(m):
    user_id = m.from_user.id
    card = m.text.replace('/str4', '').strip()
    if not card:
        bot.reply_to(m, "⚠️ /str4 number|month|year|cvv")
        return
    check_single_card(user_id, card, Stripe4, "Stripe 4")
