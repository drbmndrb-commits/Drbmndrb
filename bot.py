...
def check_mass_cards(user_id, cards, gateway_func, gateway_name, message_id, stop_key):
    ...
    approved = 0
    declined = 0
    consecutive_errors = 0  # عداد الأخطاء المتتالية (للأخطاء التقنية فقط)
    MAX_CONSECUTIVE_ERRORS = 6  # الحد الأقصى للأخطاء المتتالية
    ...
    for i, card in enumerate(cards, 1):
        ...
        try:
            ...
            is_error = not is_approved and not is_declined  # أي شيء غير Approved أو Declined يعتبر خطأ تقني
            
            if is_approved:
                approved += 1
                consecutive_errors = 0  # إعادة تعيين عند النجاح
                ...
                declined += 1
                consecutive_errors = 0  # إعادة تعيين عند الـ Declined (ليس خطأ)
                # لا نرسل رسالة لكل Declined لتجنب الإزعاج
                update_stats(user_id, False)
                
            else:
                # ===== خطأ تقني (غير Approved وغير Declined) =====
